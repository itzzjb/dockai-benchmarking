...
**Why multiplicative?** The performance term $\sum_i W_i \hat{X}_i$ is dimensionless and non-negative. Multiplying by $(1 + P_{quality})$ applies a non-negative scalar: when $P_{quality}=0$ the score is unchanged; $P_{quality}=0.05$ inflates the score by ~5%; larger lint debt scales proportionally. This preserves ordering induced by the performance term and avoids the additive case where a tiny performance score could be overwhelmed by a fixed offset. Formally, for any two methods A and B with the same $P_{quality}$, $\sum_i W_i \hat{X}_i^{(A)} < \sum_i W_i \hat{X}_i^{(B)} \implies C_{final}^{(A)} < C_{final}^{(B)}$.
